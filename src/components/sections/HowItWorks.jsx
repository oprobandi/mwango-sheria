/**
 * HowItWorks.jsx — V1.3 Light Mode
 * Clean white cards on warm section background.
 */
import SectionTag from '../ui/SectionTag.jsx'

const STEPS = [
  {
    num: '01', icon: '🗣️', title: 'Describe Your Situation',
  ...
  {
    num: '02', icon: '🤖', title: 'AI Surfaces What Applies',
  ...
  {
    num: '03', icon: '📅', title: 'Track & Stay Compliant',
  ...
  {
    num: '04', icon: '👨‍⚖️', title: 'Escalate When Needed',
  ...
  return (
    <section id="how" className="section-pad" style={{ background: '#EFE9DE' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16 items-end">
        ...
        <p className="gs-reveal text-[0.9rem] text-[#8A7860] leading-[1.9] lg:pb-2">
      ...
      <div className="lg:hidden mb-10 rounded-xl overflow-hidden h-48 relative shadow-md">
        <img
          src={PROCESS_IMAGE}
          alt="Business professionals collaborating in Nairobi"
          loading="lazy"
          decoding="async"
          className="w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[rgba(28,18,7,0.5)] to-transparent flex items-end p-4">
          <span className="text-[0.65rem] font-medium text-white tracking-[0.1em] uppercase">
            AI-Powered · Kenya-Specific · Always Current
          </span>
        </div>
      </div>

      {/* Steps — white cards on mobile, border-collapse on desktop */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-0 lg:border lg:border-[rgba(180,110,20,0.14)] lg:rounded-lg lg:overflow-hidden lg:bg-[rgba(180,110,20,0.08)]">
        {STEPS.map((step, i) => (
          <div
            key={step.num}
            className="gs-reveal bg-white lg:bg-[#F8F4EE] px-6 py-8 sm:py-10 transition-colors hover:bg-[rgba(180,110,20,0.04)] rounded-xl lg:rounded-none shadow-sm lg:shadow-none"
            style={{
              borderLeft: i > 0 ? '1px solid rgba(180,110,20,0.12)' : 'none',
            }}
          >
            <div
              className="font-serif text-[2.2rem] font-light leading-none mb-5"
              style={{ color: 'rgba(180,110,20,0.2)' }}
            >
              {step.num}
            </div>
            <div className="text-base mb-4 w-10 h-10 flex items-center justify-center bg-[rgba(180,110,20,0.06)] border border-[rgba(180,110,20,0.14)] rounded-lg">
              {step.icon}
            </div>
            <div className="text-[0.82rem] font-semibold text-[#1C1207] tracking-[0.05em] uppercase mb-3">
              {step.title}
            </div>
            <p className="text-[0.83rem] text-[#8A7860] leading-[1.8]">
              {step.desc}
            </p>
          </div>
        ))}
