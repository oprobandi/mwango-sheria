/**
 * Features.jsx  — V1.1
 * 6-card capability grid — responsive, border-collapse style.
...
export default function Features() {
  return (
    <section id="features" className="section-pad" style={{ background: '#060C07' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-end mb-14">
        ...
        <p className="gs-reveal text-[0.9rem] text-[#6B6050] leading-[1.9] lg:pt-8">
      ...
      <div className="features-grid">
        {FEATURES.map((f) => (
          <div
            key={f.title}
            className="gs-reveal relative overflow-hidden p-9 transition-colors group"
          ...
          >
            ...
            <div
              className="w-[42px] h-[42px] border border-[rgba(196,123,42,0.12)] rounded-[4px] flex items-center justify-center text-[1.1rem] mb-5 relative z-[1] transition-colors group-hover:border-[rgba(196,123,42,0.5)]"
            >
              {f.icon}
            </div>

            <div className="text-[0.82rem] font-semibold text-[#F4ECD8] tracking-[0.07em] uppercase mb-2.5 relative z-[1]">
              {f.title}
            </div>

            <p className="text-[0.82rem] text-[#6B6050] leading-[1.8] relative z-[1]">
              {f.desc}
            </p>

            <span
              className="inline-block mt-4 text-[0.6rem] font-medium text-[#C47B2A] tracking-[0.18em] uppercase border border-[rgba(196,123,42,0.25)] px-2.5 py-1 rounded-[1px] relative z-[1]"
            >
              {f.tag}
            </span>
          </div>
